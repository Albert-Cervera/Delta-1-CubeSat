...
//
// DESCRIPTION:
//> This software has been optimized for the low energy consumption
// requirements of the Delta-1 Mission. Current approaches includes
// usage of MOSFETs to cut energy from payload sensors and GPS, also
// makes use of the "LowPower" library, sleeping controller and
// peripheral.
//
// NOTES & WARNINGS
//
...
//
// ISSUE:
// Program storage space should not pass 95% or it won't execute properly.
//
// REVISION HISTORY:
...
// #define ALR_PIN 2 // Alarm Pin for low battery ack (battery gauge)
...
// uint8_t intFlagBattery = 0; // To tell if CubeSat has low battery
// Perhaps I could not use it and just read battery on checkTriggerSM()
...
void transmitData() {
  ...
  struct telemetryStruct {
    ...
    float batteryTemp = systemData.batteryTemp;
    float batteryPercentage = systemData.batteryPercentage;
  ...
  */
  ...
  switch (type) {
    ...
    case 2:  // Send RTWI system data
      ...
      auxiliarData.val2 = systemData.voltage; // gauge.readVoltage(); 
    ...
    case 5:  // Send system data as SUDO (ESM)
      ...
      auxiliarData.val1 = systemData.mode;  // SUDO save system data: Don't check validTime on SAT_B
      auxiliarData.val2 = systemData.voltage; // gauge.readVoltage(); // Sometimes this val is not getting recorded on SD card.
      auxiliarData.val3 = systemData.internalTemp;
      auxiliarData.val4 = systemData.batteryTemp;
      auxiliarData.val5 = systemData.batteryPercentage;
      auxiliarData.val6 = 0.0;
...
float getInternalTemperature() {
  ...
  //  index 0 refers to the first IC on the wire
...
void checkTriggerSM(int lastMode) {
  ...
  */
  ...
  // systemData.batteryTemp = 15.0;
  // systemData.batteryPercentage = 75.0;
  ...
  */
  ...
  /*
    ...
    Suppose CubeSat is in RTWI mode, then battery drops to 10%
    SM is activated, then CS charges via solar and now battery = 11%,
    then Cubesat enters into ESM mode. Later, battery is 24%, then still ESM.
    More later, battery is 25%, then CubeSat returns to RTWI mode.

  */

  // if (systemData.batteryPercentage <= 10.0) { // 10.0
  //   isSMTriggered = true;
  //   spacecraftMode = 0;
  // } else {  // Battery > 10%
  //   if (isSMTriggered == true) {
  //     if (systemData.batteryPercentage >= 25.0) {
  //       isSMTriggered = false;
  //       spacecraftMode = lastMode;
  //     } else {
  //       spacecraftMode = 1;  // ESM
  //     }
  //   } else { // isSMTriggered is false (battery > 10%)
  //     spacecraftMode = lastMode;
  //   }
  // }

  
  // Serial.print(systemData.batteryPercentage);
  ...
  Li-ion Batteries:
    ...
    Note: modifiy the conditions below regarding the batteryTemp, because
