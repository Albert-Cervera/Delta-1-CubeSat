...
// Note: MCC is based on a Arduino Mega 2560 R3.
//
// Notation:
...
//
// REVISION HISTORY:
// 02 April 2023 - Initial Version
//  April 2023 - Modification
...
float minBatteryTemp = 3000.0;
float minBatteryPercentage = 3000.0;
float minVoltage = 10000.0;
...
void loop() {
  ...
  */
  ...
  if (rf_driver.recv(buf, &buflen)) {  // rf_driver.recv(buf, &buflen) == 1
    ...
    if (systemData.batteryTemp < minBatteryTemp) {
      minBatteryTemp = systemData.batteryTemp;
    }    
    if (systemData.batteryPercentage < minBatteryPercentage) {
      minBatteryPercentage = systemData.batteryPercentage;
    }
    if (systemData.voltage < minVoltage) {
      minVoltage = systemData.voltage;
    }
    ...
    if (systemData.batteryTemp > maxBatteryTemp) {
      maxBatteryTemp = systemData.batteryTemp;
    }    
    if (systemData.batteryPercentage > maxBatteryPercentage) {
      maxBatteryPercentage = systemData.batteryPercentage;
    }
    if (systemData.voltage > maxVoltage) {
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          NASA favicon: https://www.nasa.gov/sites/all/themes/custom/nasatwo/images/apple-touch-icon-76x76.png
          ...
          */
          ...
          /* 
            https://www.pikpng.com/pngl/b/87-879496_nasa-logo-clipart.png
          ...
          */
          ...
          // client.println("<br />");
          client.println("<hr />");
          client.println("<H2 style='color:white;'>Health data</H2>");
          // client.println("<br />");

          client.print("<h3 style='color:white;'>CubeSat Mode: </h3>");
          client.print("<p style='color:white;'>" + String(systemData.mode) + "</p>");
          ...
          client.print("<h3 style='color:white;'>Battery Temperature: </h3>");
          client.print("<p style='color:white;'>" + String(systemData.batteryTemp) + " &#176" + "</p>");

          client.print("<p style='color:white;'>Min: " + String(minBatteryTemp) + " &#176C" + " || Max: " + String(maxBatteryTemp) + " &#176C </p>");
          ;   
          
          client.print("<h3 style='color:white;'>Battery Charge Status: </h3>");
          client.print("<p style='color:white;'>" + String(systemData.batteryPercentage) + " %" + "</p>");

          client.print("<p style='color:white;'>Min: " + String(minBatteryPercentage) + " %" + " || Max: " + String(maxBatteryPercentage) + " % </p>");
          ;

          client.print("<h3 style='color:white;'>Battery Voltage: </h3>");
          client.print("<p style='color:white;'>" + String(systemData.voltage) + " mV" + "</p>");

          client.print("<p style='color:white;'>Min: " + String(minVoltage) + " mV" + " || Max: " + String(maxVoltage) + " mV </p>");
          ;

          // client.print("<h3 style='color:white;'>Battery Voltage: </h3>");
          // client.print("<p style='color:white;'>" + String(systemData.voltage/1000) + " V" + "</p>");

          // client.print("<p style='color:white;'>Min: " + String(minVoltage/1000) + " V" + " || Max: " + String(maxVoltage/1000) + " V </p>");
          // ;


          client.print("<h3 style='color:white;'>RTC Synced: </h3>");
          // client.print(transferData.clockSynced);
          client.print("<p style='color:white;'>" + String(transferData.clockSynced) + "</p>");
