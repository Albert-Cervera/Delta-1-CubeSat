...
struct dataStruct {
  ...
  float batteryTemp; // dedicated temperature DSB18B20 sensor for batteries
...
void loop() {
  ...
  if (rf_driver.recv(buf, &buflen)) {  // rf_driver.recv(buf, &buflen) == 1
    ...
    systemData.batteryTemp = transferData.batteryTemp;
    systemData.batteryPercentage = transferData.batteryPercentage;
...
void writeFile(int flag, String fileName) {
  ...
  if (flag == 1 || flag == 4 || flag == 6) {  // Save telemetry data
    ...
    if (myFile) {
      ...
      myFile.print(String(systemData.batteryTemp) + ",");
      myFile.print(String(systemData.batteryPercentage));
      myFile.print("\n");
...
void m1b1action() {
  ...
  // tft.setTextColor(WHITE);
  // tft.setTextSize(2);
  // tft.println("Batteries Temp: " + String(systemData.batteryTemp) + "\367" + "C");

  // Remove it from here, this is just for testing
  tft.setCursor(22, 112);
  tft.setTextColor(WHITE);
  tft.setTextSize(2);
  tft.println("Battery: " + String(systemData.batteryPercentage) + "%");
