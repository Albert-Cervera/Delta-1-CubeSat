...
RH_ASK rf_driver(2000, A5, A4, 0); // speed in bps, rxPin, txPin, pttPin <-- Send data in PIN A4 and receive in PIN A5 (before: 11)
...
byte tx_buf[sizeof(commandData)] = {0}; // buffer for sending  weather data
    ...
    switch(key) {
      ...
      case '#':
        if (lastKey == 'B') {
          ...
          // Send command
          sendCommand(1.0); // ask for reset pressureGroundLevel
